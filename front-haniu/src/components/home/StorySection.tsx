...
export default function StorySection() {
  ...
  // Helper to split title and highlight dynamically from configuration
  const renderTitle = () => {
    const titleText = trans(story.title || 'Nghệ Thuật Từ');
    const highlightText = trans(story.titleHighlight || 'Những Bàn Tay');
    const part2Text = trans(story.titlePart2 || 'Thủ Công');
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="space-y-4">
            <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.25em] text-rose-500 bg-rose-500/10 dark:bg-rose-500/10 border border-rose-500/20">
              <Icon name="✨" size={10} className="animate-pulse" /> {trans(story.subtitle)}
            </span>
              ...
              <>
                ...
                <div className="absolute inset-0 flex flex-col justify-center items-center gap-4 bg-zinc-950/20 group-hover:bg-zinc-950/40 transition-colors duration-500">
                  <button
                    ...
                    className="w-16 h-16 rounded-full bg-gradient-to-br from-rose-500 to-pink-500 text-white flex items-center justify-center shadow-lg shadow-rose-500/30 scale-95 group-hover:scale-105 transition-all duration-500 cursor-pointer ring-4 ring-white/30 group-hover:ring-white/50"
                    title="Phát video"
                  >
