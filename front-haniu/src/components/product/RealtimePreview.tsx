...
export default function RealtimePreview({
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 p-4 sm:p-6 rounded-3xl space-y-4 shadow-sm w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-50 dark:border-zinc-800 gap-2">
        ...
        <div className="flex gap-1.5 overflow-x-auto">
          {showEngraving && (
            <button
              ...
              className={`px-3 py-1 rounded-lg text-[10px] font-bold transition-all whitespace-nowrap ${activeTab === 'laser'
          ...
          {showCardMessage && (
            <button
              ...
              className={`px-3 py-1 rounded-lg text-[10px] font-bold transition-all whitespace-nowrap ${activeTab === 'card'
                ? 'bg-rose-500 text-white'
                : 'bg-slate-50 text-slate-500 dark:bg-zinc-800'
                }`}
            >
      ...
      {activeTab === 'laser' && showEngraving && (
        <div className="space-y-3 w-full">
          ...
          <div className="text-[9px] text-slate-400 dark:text-zinc-500 leading-relaxed text-center">
      ...
      {activeTab === 'card' && showCardMessage && (
        <div className="space-y-3.5 w-full">
          ...
          >
            ...
            <div className="absolute inset-0 opacity-[0.03] dark:opacity-[0.01] pointer-events-none mix-blend-multiply dark:mix-blend-overlay bg-repeat bg-center" style={{ backgroundImage: `url('https://www.transparenttextures.com/patterns/lined-paper-2.png')` }} />

            {/* Header section */}
            <div 
              className="flex justify-between items-center text-[15px] sm:text-[17px] font-bold tracking-wide text-amber-900/60 dark:text-amber-400/90 pb-2 border-b border-amber-900/10 dark:border-zinc-700/80"
            ...
            <div
              className="my-4 text-slate-800 dark:text-zinc-100 text-lg sm:text-xl md:text-2xl font-semibold leading-relaxed text-center overflow-y-auto max-h-[100px] px-2 break-words select-none scrollbar-thin"
              style={{
            ...
            {/* Footer section */}
            <div className="flex items-center justify-between text-[9px] text-slate-500 dark:text-zinc-400 pt-2 border-t border-amber-900/10 dark:border-zinc-700/80">
              <span className="font-medium italic">Handwritten with love 💖</span>
              <span className="font-bold flex items-center gap-1 text-slate-650 dark:text-zinc-300">
