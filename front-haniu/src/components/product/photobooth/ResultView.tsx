      ...
      <div className="flex flex-col gap-4 max-w-sm w-full">
        <div className="text-center md:text-left">
          <div className="inline-block px-2.5 py-0.5 bg-primary-color/10 text-primary-color rounded-full text-[9px] font-black uppercase tracking-wider mb-1">
            {trans("Đã hoàn tất ảnh ghép")}
          </div>
          <h2 className="text-xl sm:text-2xl font-black text-foreground uppercase tracking-tight font-sans italic">{trans("SIÊU PHẨM XINH XẮN! 🌸")}</h2>
          <p className="text-[10px] text-muted-color">{trans("Ảnh đã ghép thành công và sẵn sàng để in.")}</p>
        </div>
        ...
        <div className="flex flex-col gap-2.5">
          {onBackToEdit && (
            ...
            >
              <Icon name="arrow-left" size={12} className="text-rose-500" /> {trans("Quay lại chỉnh sửa tiếp")}
            </button>
          )}
          ...
          >
            <Icon name="camera" size={13} className="text-rose-500" /> {trans("Đính kèm & Chụp tiếp")}
          </button>
          ...
          >
            <Icon name="check" size={14} /> {trans("Đính kèm và Đóng")}
          </button>
        </div>
        ...
        >
          ...
          <span>{trans("Chụp bộ ảnh mới")}</span>
        </button>
      </div>
    </div>
  );
