...
export default function CollectionDetailClient({ slug, initialCollection, initialProducts }: CollectionDetailClientProps) {
  ...
  useEffect(() => {
    // Skip initial load if data is already provided and sortOption is 'newest'
    if (sortOption === 'newest' && initialProducts.length > 0) {
      return;
    }
    ...
    const fetchCollectionData = async () => {
      try {
        ...
        console.error('Error fetching collection products on sort change:', err);
    ...
    fetchCollectionData();
  }, [sortOption, collection, initialProducts]);

  // Sync state with dynamic collection updates
  useEffect(() => {
    setCollection(initialCollection);
    setProducts(initialProducts);
  }, [initialCollection, initialProducts]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center py-20">
            ...
            <p className="text-xs text-muted-color">Đang sắp xếp...</p>
          </div>
