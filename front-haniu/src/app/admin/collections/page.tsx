...
export default function AdminCollectionsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setErrorMsg('Tên bộ sưu tập không được để trống.');
      return;
    }
    if (!slug.trim()) {
    ...
    try {
      ...
      setSuccessMsg(editingId ? 'Cập nhật bộ sưu tập thành công! 🎉' : 'Thêm bộ sưu tập mới thành công! 🎉');
      setTimeout(() => {
        setIsModalOpen(false);
      }, 1000);
    } catch (err: any) {
      setErrorMsg(err.message || 'Lỗi khi lưu bộ sưu tập.');
    } finally {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Bạn có chắc chắn muốn xóa bộ sưu tập này không? Các sản phẩm trong bộ sưu tập sẽ không bị xóa nhưng sẽ không còn thuộc bộ sưu tập này nữa.')) return;
    try {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 dark:border-zinc-800 pb-5">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-slate-800 dark:text-white">Quản lý Bộ sưu tập (Collections)</h1>
          <p className="text-xs text-slate-400">Tạo, cập nhật các bộ sưu tập quà tặng đặc sắc của cửa hàng</p>
        </div>
        <button
        ...
        >
          <Icon name="plus" size={14} /> Thêm Bộ Sưu Tập Mới
        </button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm w-full">
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto text-xs font-semibold">
          ...
          {(['ALL', 'ACTIVE', 'INACTIVE'] as const).map(f => (
            ...
            >
              {f === 'ALL' ? 'Tất cả' : f === 'ACTIVE' ? 'Hoạt động' : 'Tạm ẩn'}
            </button>
          ))}
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 dark:bg-zinc-850 border-b border-slate-100 dark:border-zinc-800 text-slate-500 font-bold uppercase tracking-wider text-[10px]">
                  <th className="p-4 w-20">Ảnh đại diện</th>
                  <th className="p-4 w-32">Ảnh Banner</th>
                  <th className="p-4">Tên bộ sưu tập</th>
                  ...
                  <tr key={item.id} className="border-b border-slate-50 dark:border-zinc-800 hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                    <td className="p-4">
                      {item.imageUrl ? (
                        ...
                        <div className="w-12 h-12 rounded-xl bg-amber-500/10 text-amber-500 flex items-center justify-center font-bold text-sm">
                          ✨
                        </div>
                      )}
                    </td>
                    <td className="p-4">
                      {item.bannerUrl ? (
                        <img
                          src={getFullImageUrl(item.bannerUrl)}
                          alt={`${item.name} banner`}
                          className="w-24 h-12 object-cover rounded-xl border border-slate-100 dark:border-zinc-700 bg-slate-105"
                        />
                      ) : (
                        <span className="text-slate-350 dark:text-zinc-650 italic">Không có banner</span>
                      )}
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-zinc-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-lg rounded-3xl border border-slate-100 dark:border-zinc-800 shadow-2xl p-6 relative flex flex-col max-h-[90vh] overflow-y-auto scrollbar-none animate-in fade-in zoom-in-95 duration-200 text-xs font-semibold">
            ...
            <h3 className="text-lg font-bold text-slate-800 dark:text-white border-b border-slate-100 dark:border-zinc-855 pb-3 mb-5">
              {editingId ? 'Cập nhật Bộ Sưu Tập' : 'Tạo Bộ Sưu Tập Mới'}
            </h3>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              {/* Name */}
              <div>
                <label className="text-[10px] text-slate-400 block mb-1 uppercase tracking-wider">Tên bộ sưu tập *</label>
                <input
                  type="text"
                  required
                  value={name}
                  onChange={e => handleNameChange(e.target.value)}
                  placeholder="Ví dụ: Summer Breeze Collection"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="summer-breeze-collection"
              ...
              {/* Description */}
              <div>
                <label className="text-[10px] text-slate-400 block mb-1 uppercase tracking-wider">Mô tả bộ sưu tập</label>
                <textarea
                  value={description}
                  onChange={e => setDescription(e.target.value)}
                  placeholder="Nhập vài câu giới thiệu về bộ sưu tập này..."
              ...
              <div>
                <label className="text-[10px] text-slate-400 block mb-1 uppercase tracking-wider">Hình ảnh đại diện (Thumbnail)</label>
                <div className="flex gap-4 items-center">
                  {imageUrl ? (
                    <div className="relative shrink-0 border border-slate-200 dark:border-zinc-700 rounded-xl overflow-hidden w-20 h-20 bg-slate-100">
                      <img src={getFullImageUrl(imageUrl)} alt="Thumbnail Preview" className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={() => setImageUrl('')}
                  ...
                  <div className="flex-1 space-y-1.5">
                    <input
                      type="file"
                      accept="image/*"
                      id="upload-collection-thumb"
                      onChange={e => handleImageUpload(e, false)}
                      disabled={uploadingThumbnail}
                      className="hidden"
                    />
                    <label
                      htmlFor="upload-collection-thumb"
                    ...
                    >
                      {uploadingThumbnail ? (
                        ...
                        <>
                          <Icon name="camera" size={12} /> Chọn ảnh đại diện
                        </>
                      )}
                    </label>
                    <p className="text-[10px] text-slate-400 font-medium">Khuyên dùng tỷ lệ 1:1 hoặc 4:5 (dạng tạp chí)</p>
                  </div>
                </div>
              </div>
              ...
              {/* Banner Image Upload */}
              <div>
                <label className="text-[10px] text-slate-400 block mb-1 uppercase tracking-wider">Hình ảnh bìa lớn (Banner)</label>
                <div className="flex gap-4 items-center">
                  {bannerUrl ? (
                    <div className="relative shrink-0 border border-slate-200 dark:border-zinc-700 rounded-xl overflow-hidden w-28 h-16 bg-slate-100">
                      <img src={getFullImageUrl(bannerUrl)} alt="Banner Preview" className="w-full h-full object-cover" />
                      <button
                        type="button"
                    ...
                    <div className="w-28 h-16 bg-slate-50 dark:bg-zinc-800 border border-dashed border-slate-200 dark:border-zinc-700 rounded-xl flex flex-col items-center justify-center text-slate-400 gap-1 shrink-0">
                      <Icon name="camera" size={16} />
                      <span className="text-[8px] font-bold">Chưa có ảnh bìa</span>
                    </div>
                  )}
                  ...
                  <div className="flex-1 space-y-1.5">
                    <input
                      type="file"
                      accept="image/*"
                      id="upload-collection-banner"
                      onChange={e => handleImageUpload(e, true)}
                      disabled={uploadingBanner}
                      className="hidden"
                    />
                    <label
                      htmlFor="upload-collection-banner"
                      className="px-4 py-2 border border-slate-200 dark:border-zinc-700 hover:bg-slate-50 dark:hover:bg-zinc-800 bg-white dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 font-bold rounded-lg cursor-pointer transition-all inline-flex items-center gap-1.5 active:scale-95"
                    >
                      {uploadingBanner ? (
                        <>
                        ...
                        <>
                          <Icon name="camera" size={12} /> Chọn ảnh bìa
                        </>
                      )}
                    </label>
                    <p className="text-[10px] text-slate-400 font-medium">Được hiển thị làm banner trang chi tiết bộ sưu tập</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <label htmlFor="isActive" className="text-slate-700 dark:text-zinc-300 font-bold cursor-pointer">
                  Kích hoạt hoạt động (Hiển thị trên trang chủ & bộ lọc)
                </label>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 dark:border-zinc-850 mt-6">
                ...
                >
                  {submitLoading ? (
                    ...
                    <>
                      <Icon name="save" size={12} /> Lưu bộ sưu tập
                    </>
                  )}
                </button>
              </div>
